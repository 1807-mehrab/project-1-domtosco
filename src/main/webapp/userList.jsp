...
<html>
	...
	<body>
		...
		<div style="background-color:lightblue">
			<h2><c:out value="User List" /></h2>
		</div>
		
		<c:forEach var="usr" items="${userSet}">
			<div class="boxyBox" style="background-color:lightblue">
				<h2><c:out value="${usr.userID}" /></h2>
				<form class="formy2" action="ShowPosts" method="get">
					<input type="hidden" name="flagged" value="false">
					<input type="hidden" name="userName" value="${usr.userID}">
					<button class="pages">User Posts</button>
				</form>
			</div>
		</c:forEach>
	</body>
